...
RafRaf's notification channel is a Telegram bot — free, official, and no
approval/business verification needed. It's the **only** channel (WhatsApp/Green
API was removed).

What Telegram delivers: the **merchant's own** alerts — nightly daily summary,
instant low-stock alert, and (to the admin) backup-failure alerts — plus
**customer debt reminders**. Because Telegram can only message a person who has
first started your bot, a customer must link their Telegram: they DM the bot
`/start`, then the owner pastes the returned chat id into the customer's profile
(**Customers → [customer] → edit → Telegram chat ID**).
...
`x-webhook-secret: <LOW_STOCK_WEBHOOK_SECRET>`. It fires (over Telegram) the
moment stock crosses below the alert threshold during a sale's sync.
...
- **Customer reminder:** open a customer with debt → **Remind via Telegram**. If
  they have no linked chat id you'll see "الزبون ما عنده تيليغرام مربوط".
